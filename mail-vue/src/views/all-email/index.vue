<template>
  <div class="email-split" :class="{ 'has-detail': selectedEmail, 'is-resizing': isResizing }">
          ...
          <el-input v-model="searchValue" :placeholder="$t('searchByContent')" class="search-input">
            <template #prefix>
              <div @click.stop="openSelect">
                <el-select ref="mySelect" v-model="params.searchType" :placeholder="$t('select')" class="select">
    ...
    <el-dialog v-model="showBathDelete" :title="$t('clearEmail')" width="335" @closed="closedClear">
      <div class="clear-email">
        ...
        <el-date-picker popper-class="my-date-picker" v-model="clearTime" type="daterange"
                        :teleported="false" unlink-panels :range-separator="t('to')" size="default"/>
...
const params = reactive({
  timeSort: 0, type: 'receive', userEmail: null, accountEmail: null, name: null, subject: null, searchType: 'name'
})

const clearParams = reactive({
  subject: '', sendEmail: '', sendName: '', startTime: '', toEmail: '', endTime: '', type: 'eq',
})

function resetClearParams() {
  clearParams.subject = ''; clearParams.sendEmail = ''; clearParams.sendName = '';
  clearParams.startTime = ''; clearParams.toEmail = ''; clearParams.endTime = '';
}

function closedClear() { resetClearParams(); clearParams.type = 'eq'; clearParams.endTime = ''; clearTime.value = null }
...
if (paramsStar) {
  ...
  params.type = locaParams.type; params.timeSort = locaParams.timeSort
  params.status = locaParams.status; params.searchType = locaParams.searchType
}

watch(() => params, () => { localStorage.setItem('all-email-params', JSON.stringify(params)) }, { deep: true })

function openBathDelete() { showBathDelete.value = true }
...
function batchDelete() {
  ...
  if (!clearParams.sendEmail && !clearParams.sendName && !clearParams.subject && !clearParams.toEmail && !clearTime.value) {
    showBathDelete.value = false; return
  }
  ElMessageBox.confirm(t('delAllConfirm'), {
    ...
    allEmailBatchDelete(clearParams).then(() => {
      ...
      resetClearParams(); sysEmailScroll.value.refreshList();
    }).finally(() => { clearLoading.value = false })
  })
}

function rightSearch(type, value) { params.searchType = type; searchValue.value = value; search(); }

function refreshBefore() {
  searchValue.value = null; params.timeSort = 0; params.type = 'receive';
  params.userEmail = null; params.accountEmail = null; params.name = null; params.subject = null; params.searchType = 'name'
}

function search() {
  params.userEmail = null; params.accountEmail = null; params.name = null; params.subject = null
...
function changeTimeSort() { params.timeSort = params.timeSort ? 0 : 1; search() }
function typeSelectChange() { search() }

function onJump(email) {
  setEmail(email, { delType: 'physics', showStar: false, showReply: false, showUnread: false })
}

function onDblClick(email) {
  dblClickContent(email, { delType: 'physics', showStar: false, showReply: false, showUnread: false })
}
...
@media (max-width: 767px) {
  .el-date-range-picker .el-picker-panel__body { min-width: auto; }
  .my-date-picker::after { content: ""; position: absolute; left: 0; right: 0; height: 20px; background: transparent; }
  .el-date-range-picker__content { width: 100%; }
  .el-date-range-picker { width: 300px; }
  .el-tooltip .el-picker_popper { padding-bottom: 200px; }
  .el-date-range-picker__content.is-left { border-right: 0; }
}
</style>
...
.email-split {
  ...
  .select { position: absolute; width: 40px; opacity: 0; pointer-events: none; }
  .search-type { display: flex; color: var(--el-text-color-regular); }
  :deep(.header-actions) { padding-top: 8px; padding-bottom: 8px; }
  .clear-email { display: flex; flex-direction: column; gap: 15px; }
  .clear-button { display: flex; align-items: center; gap: 15px; .el-button { width: 100%; } }
  .status-select { margin-bottom: 2px; width: 102px; :deep(.el-select__wrapper) { min-height: 28px; } }
  .input-with-select { max-width: 200px; border-radius: 0 4px 4px 0; }
  :deep(.input-with-select .el-input-group__append) { background-color: var(--el-fill-color-blank); }
  :deep(.el-select__wrapper) { padding: 2px 10px; min-height: 28px; }
  :deep(.el-date-editor.el-input__wrapper) { width: 303px; }
  .icon { cursor: pointer; }
  .clear { @media (max-width: 419px) { position: absolute; top: 41px; left: 242px; } }
  :deep(.reload) { @media (max-width: 419px) { position: absolute; top: 42px; left: 208px; } }
  :deep(.delete) {
